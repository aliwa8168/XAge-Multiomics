...
class ProjectionLog1p(nn.Module):
    def __init__(self, n_embed: int):
        super().__init__()
        self.count_embedding = nn.Linear(1, n_embed)

    def forward(self, genes: torch.Tensor, counts: torch.Tensor) -> torch.Tensor:
        counts = self.count_embedding(torch.log1p(counts))
        return genes + counts
...
class InputTransformerAE(nn.Module):
    def __init__(
        self,
        n_genes: int,
        n_embed: int,
        agg_func: Literal["log1p", "anscombe", "sqrt", "proj", "projlog1p", "projconcat", "softbin", "log1pzero"],
        n_atac: int | None = None,
        atac_agg_func: Literal[
            "log1p", "anscombe", "sqrt", "proj", "projlog1p", "projconcat", "softbin", "log1pzero"
        ] = "proj",
    ):
        super().__init__()
        ...
        if agg_func in ["proj", "projlog1p", "projconcat", "softbin"]:
            self.projection = PROJ_FUNC[agg_func](n_embed)

        self.peak_embedding: nn.Embedding | None = None
        self.atac_projection: nn.Module | Callable[[Any], Any] | None = None
        if n_atac is not None:
            self.peak_embedding = nn.Embedding(n_atac + 1, n_embed)
            self.atac_projection = PROJ_FUNC[atac_agg_func]
            if atac_agg_func in ["proj", "projlog1p", "projconcat", "softbin"]:
                self.atac_projection = PROJ_FUNC[atac_agg_func](n_embed)

    def token_embedding(self, tokens: torch.Tensor, modality: Literal["rna", "atac"] = "rna") -> torch.Tensor:
        """Return token embeddings for decoder queries without value projection."""
        if modality == "rna":
            return self.gene_embedding(tokens)
        if modality == "atac":
            if self.peak_embedding is None:
                raise ValueError("ATAC token embedding requested but n_atac was not configured")
            return self.peak_embedding(tokens)
    ...
    def forward(
        ...
        if counts is None or genes is None:
            raise ValueError("counts and genes/tokens must both be provided to InputTransformerAE")
        if counts.shape != genes.shape:
            raise ValueError(f"counts shape {tuple(counts.shape)} must match token shape {tuple(genes.shape)}")
        if modality == "rna":
            genes_emb = self.gene_embedding(genes.long())
            output = self.projection(genes_emb, counts.unsqueeze(-1))
        elif modality == "atac":
            if self.peak_embedding is None or self.atac_projection is None:
                raise ValueError("ATAC input projection requested but n_atac was not configured")
            genes_emb = self.peak_embedding(genes.long())
            output = self.atac_projection(genes_emb, counts.float().unsqueeze(-1))
        else:
            raise ValueError(f"Unsupported modality: {modality}")
        return output
...
class CrossAttention(nn.Module):
    ...
    def forward(
        ...
        if key_padding_mask is not None:
            ...
            attn_mask = attn_mask.masked_fill(~valid.view(B, 1, 1, S), float("-inf"))
...
class CrossAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        if self.use_adaln:
            ...
            x = q + gate_attn * self.attn(norm1_xout, norm1_qout, key_padding_mask=key_padding_mask)
            norm2_out = modulate(self.ln_2(x), scale_mlp, shift_mlp)
            x_ = gate_mlp * self.mlp(norm2_out)
            x = x + x_
        else:
            attn_output = self.attn(self.ln_1(x), self.ln_1q(q), key_padding_mask=key_padding_mask)
            x = q + attn_output
            x_ = self.mlp(self.ln_2(x))
            x = x + x_
        return x
